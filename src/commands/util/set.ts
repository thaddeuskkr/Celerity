import {
    ...
    RoleSelectMenuInteraction, PermissionFlagsBits
} from 'discord.js';
        ...
        if (args.length) {
            ...
            if (foundSetting.name === 'announce connect') {
                        ...
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions(
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('true')
                                    .setValue('true')
                                    .setDescription('Enables connect notifications.'),
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('false')
                                    .setValue('false')
                                    .setDescription('Disables connect notifications.')
                            )
                    );
                ...
                collector.on('collect', (i: StringSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ settings.announceConnect }\`.`) ],
                        components: []
                    });
                });
                        ...
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions(
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('true')
                                    .setValue('true')
                                    .setDescription('Enables disconnect notifications.'),
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('false')
                                    .setValue('false')
                ...
                collector.on('collect', (i: StringSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ settings.announceDisconnect }\`.`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                        ...
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions(
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('true')
                                    .setValue('true')
                                    .setDescription('Enables now playing messages.'),
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('false')
                                    .setValue('false')
                                    .setDescription('Disables now playing messages.')
                            )
                    );
                const message = await context.channel.send({ embeds: [ embed ], components: [ stringRow ] });
                const collector = message.createMessageComponentCollector({ filter, time: 120000, max: 1 });
                collector.on('collect', (i: StringSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ settings.announceNowPlaying }\`.`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                        ...
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions(
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('true')
                                    .setValue('true')
                                    .setDescription('Enables autoplay.'),
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('false')
                                    .setValue('false')
                                    .setDescription('Disables autoplay.')
                            )
                    );
                const message = await context.channel.send({ embeds: [ embed ], components: [ stringRow ] });
                const collector = message.createMessageComponentCollector({ filter, time: 120000, max: 1 });
                collector.on('collect', (i: StringSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ settings.autoplay }\`.`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                ...
                collector.on('collect', (i: UserSelectMenuInteraction) => {
                    ...
                    else for (let x = 0; x < newUsers.length; x++) {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | **The following users / roles are banned from using Celerity in this server:**\n${ settings.banned.length ? response.join(', ') : 'None' }`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                ...
                collector.on('collect', async () => {
                    ...
                    await message.edit({
                        embeds: [ new EmbedBuilder()
                            .setDescription(`${ client.config.emojis.loading } | **Type and send the color you want Celerity to use for embeds here.**\nAccepts a hex string, not case-sensitive. (e.g. #CBA6F7)`)
                    ...
                    messageCollector.on('collect', async (msg: Message) => {
                        ...
                        if (/^#?[\da-f]{6}$/i.test(color)) {
                            ...
                            message.edit({
                                embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ settings.color }\`.`) ],
                                components: []
                            });
                            return;
                        } else {
                            message.edit({ embeds: [ successEmbed.setColor('#F38BA8').setDescription(`${ client.config.emojis.error } | **Invalid HEX code.**`) ] });
                            return;
                        }
                    });
                ...
                embed
                    .setDescription(
                        ...
                        **----------**
                        **Default value:** No default value
                        **Current value:** ${ settings.disabledChannels.length ? settings.disabledChannels.map(c => `<#${ c }>`).join(', ') : 'No disabled channels / categories' }`
                    );
                channelRow
                    .addComponents(
                        new ChannelSelectMenuBuilder()
                            .setCustomId('set')
                            .setPlaceholder('Select up to 25 channels...')
                            .setMinValues(1)
                            .setMaxValues(25)
                    );
                ...
                collector.on('collect', (i: ChannelSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | **Celerity will no longer work in the following channels / categories:**\n${ settings.disabledChannels.length ? settings.disabledChannels.map(c => `<#${ c }>`).join(', ') : 'None' }`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                ...
                collector.on('collect', async () => {
                    ...
                    await message.edit({
                        embeds: [ new EmbedBuilder()
                            .setDescription(`${ client.config.emojis.loading } | **Type and send the timeout you would like here.**\nAccepts a number from 0 to 3600.`)
                            .setColor('#F5C2E7') ], components: []
                    });
                    messageCollector.on('collect', async (msg: Message) => {
                        ...
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions(
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('true')
                                    .setValue('true')
                                    .setDescription('Enables DJ only mode.'),
                                new StringSelectMenuOptionBuilder()
                                    .setLabel('false')
                                    .setValue('false')
                                    .setDescription('Disables DJ only mode.')
                            )
                    );
                const message = await context.channel.send({ embeds: [ embed ], components: [ stringRow ] });
                const collector = message.createMessageComponentCollector({ filter, time: 120000, max: 1 });
                ...
                collector.on('collect', (i: RoleSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to ${ settings.dj.role.length > 0 ? `<@&${ settings.dj.role }>` : '`none`' }.`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                ...
                embed
                    .setDescription(
                        ...
                        If you didn't already know, prefixes are what command messages start with, that Celerity will recognise. For example, in your invocation message, \`${ client.util.escapeBackticks(prefix.replace(/<@!?\d+>/g, `@${ client.user!.tag } `)) }\` was your prefix.*
                        **----------**
                ...
                collector.on('collect', async () => {
                    ...
                    messageCollector.on('collect', async (msg: Message) => {
                        ...
                        message.edit({
                            embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | **Celerity will now respond to commands prefixed by the following:**\n${ prefixes.map(prefix => `- \`${ client.util.escapeBackticks(prefix) }\``).join('\n') }`) ],
                            components: []
                        });
                        return;
                    });
                ...
                collector.on('collect', (i: StringSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ resolveProviderToString(settings.searchProvider) }\`.`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
                ...
                collector.on('collect', (i: StringSelectMenuInteraction) => {
                    ...
                    message.edit({
                        embeds: [ successEmbed.setDescription(`${ client.config.emojis.success } | Set **${ foundSetting.name }** to \`${ settings.setStageTopic }\`.`) ],
                        components: []
                    });
                });
                collector.on('end', async collected => {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    ...
                    `To modify a setting or view more information about it, use \`${ client.util.escapeBackticks(prefix.replace(/<@!?\d+>/g, `@${ client.user!.tag } `)) }set <setting>\`, where \`<setting>\` is the name of the setting you want to change, found below.\n`
                )
                .setFields(
                    {
