...
export const command: Command = {
    ...
    async execute({ client, context, player, settings }) {
        ...
        if (!settings.announceNowPlaying) client.respond(context.channel, `${ client.config.emojis.previous } | **Returned to [${ prev!.info.title } by ${ prev!.info.author }](${ prev!.info.uri }).**`, 'success');
    }
};