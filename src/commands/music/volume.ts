...
export const command: Command = {
    ...
    async execute({ client, context, args, player }) {
        if (!args.length) return client.respond(context.channel, `${ client.config.emojis.volume } | **Currently playing at ${ (player.player.filters.volume || 1) * 100 }% volume.**`, 'info');
        else {
            ...
            if (newVolume > 250 || newVolume < 0) return client.respond(context.channel, `${ client.config.emojis.error } | **Invalid integer.**\nAccepts: \`0 - 100\`.`, 'error');
