...
export const command: Command = {
    ...
    async execute({ client, context, settings, args, prefix }) {
        ...
        if (cmd) {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    {
                        name: 'Usage:',
                        value: '`' + `${ client.util.escapeBackticks(prefix.replace(/<@!?\d+>/g, `@${ client.user!.tag } `)) }${ command.name }${ command.options?.length ? ' ' : '' }` + command.options?.map((option) => `${ option.required ? `<${ option.name }>` : `[${ option.name }]` }`).join(' ') + '`'
                    },
                    {
                        name: 'Options:',
                        value: command.options?.length ? command.options.map((option) => `**\`${ option.name }\` ${ option.required ? '(Required) ' : '' }- ${ ApplicationCommandOptionType[option.type] }:** ${ option.description }${ option.choices ? ` (${ option.choices.length } choices)` : '' }`).join('\n') : 'None'
                    }
                )
        ...
        const musicMap = client.commands.filter(c => c.category === 'music').map(
        ...
        paginatedMessage.addPageEmbed(
            new EmbedBuilder()
                ...
                .setDescription(client.commands.filter(c => c.category === 'info').map(
                    c => `**${ c.name }** - ${ c.description }`
                ).join('\n'))
        );
        paginatedMessage.addPageEmbed(
            new EmbedBuilder()
                .setTitle('Music Commands [1/2]')
                .setDescription(musicCommands[0]!.join('\n'))
        );
        paginatedMessage.addPageEmbed(
            new EmbedBuilder()
                .setTitle('Music Commands [2/2]')
                .setDescription(musicCommands[1]!.join('\n'))
        );
        paginatedMessage.addPageEmbed(
            new EmbedBuilder()
                .setTitle('Utility Commands')
                .setDescription(client.commands.filter(c => c.category === 'util').map(
                    c => `**${ c.name }** - ${ c.description }`
                ).join('\n'))
        );
