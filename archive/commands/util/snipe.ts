...
export const command: Command = {
    ...
    async execute({ client, context, settings }) {
        ...
        if (!channelSnipes.length)
            return client.respond(context.channel, `${client.config.emojis.error} | **No deleted messages recorded in this channel.**`, 'error');
        const paginatedMessage = new CelerityPaginatedMessage(client, {
            template: new EmbedBuilder().setColor(settings.color),
        });
